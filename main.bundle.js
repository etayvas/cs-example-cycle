...
const run_1 = __webpack_require__(39);
const dom_1 = __webpack_require__(7);
const http_1 = __webpack_require__(104);
const search_1 = __webpack_require__(115);
const buttons_1 = __webpack_require__(119);
__webpack_require__(120);
function main(sources) {
    const search$ = search_1.default({ dom: sources.dom, http: sources.http }), buttons$ = buttons_1.default({ dom: sources.dom }), dom$ = xstream_1.default.combine(search$.dom, buttons$.dom)
        .map(([searchDOM, buttonsDOM]) => {
        return dom_1.div(".main-holder", [
            ,
            dom_1.div('SC CycleJS'),
            searchDOM,
            buttonsDOM
        ]);
    }), sinks = {
...
var selectorParser_1 = __webpack_require__(23);
...
var xstream_1 = __webpack_require__(0);
var adapt_1 = __webpack_require__(3);
var MainHTTPSource_1 = __webpack_require__(106);
var superagent = __webpack_require__(108);
...
var RequestBase = __webpack_require__(110);
var isObject = __webpack_require__(17);
var isFunction = __webpack_require__(111);
var ResponseBase = __webpack_require__(112);
var shouldRetry = __webpack_require__(114);
...
Object.defineProperty(exports, "__esModule", { value: true });
const xstream_1 = __webpack_require__(0);
const debounce_1 = __webpack_require__(116);
const dom_1 = __webpack_require__(7);
const sc_1 = __webpack_require__(117);
const history_1 = __webpack_require__(118);
SC.initialize({
...
function intent(sources) {
    return {
        typedSearch$: sources.dom.select(".search-input").events("keyup")
            ...
            .compose(debounce_1.default(800)) //delay when typing
            .startWith(""),
        ...
        clickOnHistory$: sources.dom.select(".history-items div").events("click")
            .map(event => {
            return event.currentTarget.getAttribute("data-item") !== null
...
function Search(sources) {
    const actions = intent(sources), cid = "ggX0UomnLs0VmW7qZnCzw"
    //either input or history item
    , userInput$ = xstream_1.default.merge(actions.typedSearch$, actions.clickOnHistory$), requestTrackList$ = xstream_1.default.combine(userInput$, actions.clickOnNext$)
    ...
    }), requestSingleTrack$ = xstream_1.default.combine(userInput$, actions.clickOnTrack$)
    ...
    }), http$ = xstream_1.default.merge(requestTrackList$, requestSingleTrack$), resList$ = sources.http.select('list')
        .flatten()
        .map(res => res.body)
        .startWith(null), list$ = xstream_1.default.combine(resList$)
        .map(([list]) => {
        return list === null ? null : sc_1.SetList(list.collection);
    }), resTrack$ = sources.http.select('track')
        .flatten()
        .map(trackData => {
        return trackData.body ? sc_1.SetTrack(trackData.body) : dom_1.div();
    }).startWith(dom_1.div()), history$ = history_1.default({ dom: sources.dom }), vtree$ = xstream_1.default.combine(userInput$, list$, resTrack$, history$.dom)
        .map(([userInput, listDOM, trackDOM, historyDOM]) => {
        return dom_1.div(".search-holder", [
            dom_1.div('.search-field', [
                dom_1.input('.search-input', { attrs: { type: 'text', name: 'search-input', placeholder: 'Type to search', value: '', autofocus: "autofocus" } })
            ]),
            dom_1.div('.search-results', [
                ...(!userInput
                    ? [dom_1.div()]
                    : [listDOM, trackDOM])
            ]),
...
Object.defineProperty(exports, "__esModule", { value: true });
const dom_1 = __webpack_require__(7);
let currentTrack = "", player;
//not used
function SetIcon(type) {
    let icon_play = "https://cdn4.iconfinder.com/data/icons/ionicons/512/icon-play-128.png", icon_pause = "https://cdn4.iconfinder.com/data/icons/ionicons/512/icon-pause-128.png";
    return type === "play"
        ? dom_1.div(".paused", dom_1.img({ attrs: { src: icon_play } }))
        : dom_1.div(".playing", dom_1.img({ attrs: { src: icon_pause } }));
}
//create the list from received data
function SetList(resCollection) {
    const items = resCollection.map((item, index) => {
        return dom_1.div('.track-' + index, { attrs: { id: item.id } }, item.title);
    }), trackList = dom_1.div('.track-list', items);
    return trackList;
}
...
function SetTrack(trackData) {
    ...
    (player === undefined || currentTrack !== trackData.id)
        ? [
            SC.stream('/tracks/' + trackData.id).then(function (stream) {
                player = stream;
    ...
    return dom_1.div('.track-data', [
        ...
        //         )
        //       ,
        dom_1.div(".title", trackData.title)
        //, div(".description",TrackData.description)
        ,
        dom_1.div(".created", trackData.created_at),
        trackData.artwork_url ? dom_1.img(".artwork", { attrs: { src: trackData.artwork_url.replace("-large.jpg", "-t250x250.jpg") } }) : "[NO IMAGE]"
...
const xstream_1 = __webpack_require__(0);
const dom_1 = __webpack_require__(7);
let LSStatus = false, prevInput = "";
localStorage && localStorage.getItem('history')
    ? console.log("LS exist")
    : localStorage.setItem('history', "[ ]"); // first init if not exist
function SHistory(sources) {
    const input$ = sources.dom.select(".search-input").events("input")
        .map(event => event.target.value)
        .startWith(""), clickOnTrack$ = sources.dom.select(".track-list div").events("click")
        .map(event => {
        LSStatus = true;
        return true;
    })
        .startWith(false), updateLS$ = xstream_1.default.combine(clickOnTrack$, input$)
        .map(([track, input]) => {
    ...
    }), vtree$ = xstream_1.default.combine(updateLS$)
        .map(([update]) => {
        let getHistory = JSON.parse(localStorage.getItem('history')).length; // manipulate according to LS
        return dom_1.div(".search-history", [
            ,
            getHistory > 0 ? dom_1.div("Recent Searches: ") : "" // decide if to show title
            ,
            dom_1.div(".history-items", getHistory > -1 // show items if exists
                ? (JSON.parse(localStorage.getItem('history')))
                    .map((item) => {
                    return dom_1.div({ dataset: { item: item } }, item);
                })
                : dom_1.div())
        ]);
    }), sinks = {
...
function Button(sources) {
    const vtree$ = xstream_1.default.of(dom_1.div(".buttons-area", [
        dom_1.button('.button-next', "Next")
